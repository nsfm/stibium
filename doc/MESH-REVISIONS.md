...
| crowd3-r1 | level-3 crowding grant, two-signal rule (DEFAULT ON; MAX=3 referee round) | 1.02M | 0 | 1679 | 27.4K | **0.217** | THE ETERNAL WORST SITE MOVED (0.364 since autod32 -> 0.217, new worst (-28.65, -23.56, 17.84)); 1,472 of 7,882 leaves @3 (19%, targeted); bill 26.5 min / 6.99 GB (1.75x tris, 1.9x time, 2x RAM); screws tilt 4.03 -> 3.13% (Nate: base "hideous", grant variants identical); bino worst 0.091 best-ever + Nate-verified visual improvement; CROWD_MAX=2 reverts |
| night-r1 | + smooth-branch curve trigger (QUILT SOLVED) + hidden-cell witnesses (dense-details class) | 1.03M | 0 | 1654 | 27.4K | 0.217 | the V-quilt region 125 -> 876 tris, max tri area 0.813 -> 0.061 (Nate's coordinate, DENS-mapped: 7 smooth leaves at level 0 beyond fill reach); 302 thin cells certified -> 4,140 proof-carrying witnesses at model scale; 25.9 min / 7.13 GB; screws worst 0.177 -> **0.061** (under the product bar), tilt 2.99%/0.662 deg = round best; opus review: no BUG/RISK findings; next hunt: the 0.217 site (-28.65, -23.56, 17.84) - the same region whose tseed step-rejects read mc-grad 5/sp |
| face-audit | FACE metric era: default = witnesses + quilt fix, CROWD_MAX=2 (grant -> detail dial) | 578K | 0 | 424 | 27.2K | 0.217 | THE FOUR-WAY FACE A/B: curve_r1 540K @ 0.052% deviant area / **DEFAULT 578K @ 0.022%, 14.7 min, 3.87 GB** / grant 1.03M @ 0.020% (statistically equal, 2x bill) / grant+QEM0.01 271K @ 0.022% but mean deviation 4x worse (0.0020 sp) + worst face 0.635 sp + full 27.5 min / 7.2 GB upstream bill - Nate's QEM doubt confirmed by instruments; new boss: shared worst face (-5.26, 53.5, 68.0) 0.39 sp, resolution-immune |
...
| STITCH-r1 | law-generative wall stitch, default on (third step round) | 135K | 0 | 310 | 23762 | 0.237 | bino_STITCH.stl; one-variable A/B vs STITCH=0 same HEAD: FACE 0.069% = 0.069%, nm 310 = 310, repairs 7467 = 7467, FACE cluster diff ZERO (the STEPDENSE collateral does not exist - stitch is post-extraction, no lattice phase); census 8 pairs licensed / 20 ribbons / 581 tris; extract seam1 off-law 53 -> 19, std 0.0091 -> 0.0050, seam2 17 -> 5 (bino_extract_STITCH.stl); residual: 6 deep rows at the extract's 2 pre-existing nm pinch stations (tet-ring wedge pairing = the follow-up) |
| STITCH-r2 | tet-ring sheet-mate walk + adaptive certification + face-touching bar 0.008 (Nate's eye round: gaps + flat-surface riders) | 135K | 0 | 310 | 23762 | 0.237 | bino_STITCH2.stl (+ _sites.txt overlay records); extract: seam1 off-law 53 -> **3**, worst air 0.030 -> **0.006**, std 0.0091 -> **0.0014** (= seam2 texture), seam2 17 -> 5, deep airs **0**, ONE contiguous ribbon per seam, **nm 2 -> 0** (the stitch heals the pinches); bino A/B still collateral-free (FACE 0.069% = 0.069%, nm 310 = 310, repairs =, cluster deltas 0); rail-pure emission exact (worst 1.6e-5 sp), face-touching capped 0.0078; battery: ladder r1/r2 PASS, cyl r1/r2 row-identical, screws FINALBOSS, r7 disengage exact, suites green |
